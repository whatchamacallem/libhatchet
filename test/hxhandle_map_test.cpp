...
TEST_F(hxhandle_map_f, hxhandle_map_insert_and_get) {
	...
	EXPECT_EQ(v0->id, (int32_t)0);
	EXPECT_EQ(v1->id, (int32_t)1);
	EXPECT_EQ(v2->id, (int32_t)2);
...
TEST_F(hxhandle_map_f, hxhandle_map_insert_after_erase_reuses_slot_with_new_generation) {
	...
	EXPECT_NE(reused, h1);
	EXPECT_EQ(m.get(reused)->id, (int32_t)11);
...
TEST_F(hxhandle_map_f, hxhandle_map_full_cycle_reuse_across_all_slots) {
	...
	EXPECT_EQ(m.size(), (hxsize_t)3);
	EXPECT_EQ(m.get(r0)->id, (int32_t)10);
	EXPECT_EQ(m.get(r1)->id, (int32_t)11);
	EXPECT_EQ(m.get(r2)->id, (int32_t)12);
}
...
TEST_F(hxhandle_map_f, hxhandle_map_clear_multiple_values_then_reinsert) {
	...
	EXPECT_EQ(m.size(), (hxsize_t)3);
	EXPECT_EQ(m.get(r0)->id, (int32_t)20);
	EXPECT_EQ(m.get(r1)->id, (int32_t)21);
	EXPECT_EQ(m.get(r2)->id, (int32_t)22);
}

TEST_F(hxhandle_map_f, hxhandle_map_destructor_destroys_all_values) {
	{
		const hxhandle_map<hxtest_object, 2> m;
		EXPECT_TRUE(check_totals((hxsize_t)0));
	}
	EXPECT_TRUE(check_totals((hxsize_t)0));
	{
		hxhandle_map<hxtest_object, 2> m;
		m.insert(0);
		EXPECT_EQ(m_constructed, (hxsize_t)1);
		EXPECT_EQ(m_destructed, (hxsize_t)0);
	}
	EXPECT_TRUE(check_totals((hxsize_t)1));
	{
		hxhandle_map<hxtest_object, 2> m;
		m.insert(0);
		m.insert(1);
		m.insert(2);
		EXPECT_EQ(m_constructed, (hxsize_t)4);
		EXPECT_EQ(m_destructed, (hxsize_t)1);
	}
	EXPECT_TRUE(check_totals((hxsize_t)4));
}
...
TEST_F(hxhandle_map_f, hxhandle_map_set_size_bits_allocates) {
	...
	EXPECT_EQ(m.size(), (hxsize_t)3);
	EXPECT_EQ(m.get(h0)->id, (int32_t)30);
	EXPECT_TRUE(m.erase(h1));
	EXPECT_EQ(m.size(), (hxsize_t)2);
	EXPECT_NE(m.get(h0), static_cast<hxtest_object*>(hxnull));
	EXPECT_NE(m.get(h2), static_cast<hxtest_object*>(hxnull));
...
TEST_F(hxhandle_map_f, hxhandle_map_erase_if_removes_matching_values) {
	...
	for(hxsize_t i = 0; i != m.size(); ++i) {
		if(m.data()[i].id == 1) { found1 = true; }
		if(m.data()[i].id == 3) { found3 = true; }
	}
	EXPECT_TRUE(found1);
	EXPECT_TRUE(found3);
	...
	EXPECT_NE(r4, h4);
	EXPECT_EQ(m.get(r4)->id, (int32_t)14);
	EXPECT_EQ(m.size(), (hxsize_t)3);
}
...
TEST_F(hxhandle_map_f, hxhandle_map_erase_if_removes_last_value_only) {
	...
	m.insert(1);
	const hxhandle_t h2 = m.insert(2);
	const hxsize_t removed = m.erase_if([](const hxtest_object& x) -> bool { return x.id == 2; });
...
TEST_F(hxhandle_map_f, hxhandle_map_erase_if_matches_histogram_of_random_sequence) {
	...
	hxhandle_map<hxtest_ref_tracker_t, 8> m;
	hxrandom rng(777u);
	...
	const hxsize_t removed = m.erase_if([&](const hxtest_ref_tracker_t& x) -> bool {
...
TEST_F(hxhandle_map_f, hxhandle_map_random_mutation_stays_valid) {
	...
	hxhandle_map<hxtest_ref_tracker_t, 4> m;
	...
	for(int step = 0; step < 20000; ++step) {
		...
		if(should_insert) {
			...
			const hxsize_t victim = rng.range(static_cast<hxsize_t>(0), live.size());
			const hxhandle_t h = live[victim].handle;
			EXPECT_TRUE(m.erase(h));
			EXPECT_EQ(m.get(h), static_cast<hxtest_ref_tracker_t*>(hxnull));
		...
		for(hxsize_t i = 0; i != live.size(); ++i) {
			hxtest_ref_tracker_t* const v = m.get(live[i].handle);
			ASSERT_NE(v, static_cast<hxtest_ref_tracker_t*>(hxnull));
			EXPECT_EQ(v->value, live[i].value);
		}

		const hxtest_ref_tracker_t* const data = m.data();
		for(hxsize_t i = 0; i != live.size(); ++i) {
			bool found = false;
