...
class hxvector_test_f* hxs_test_current = hxnull;

class hxvector_test_f :
	public testing::Test
{
public:
	class hxtest_object {
	...
	~hxvector_test_f(void) {
		hxassert_always(m_constructed == m_destructed, "hxvector_test_f Test object lifecycle error.");
		hxs_test_current = 0;
	}
...
template<typename T>
class hxvector_test_pointer_range {
public:
	hxvector_test_pointer_range(T* b, T* e)
...
TEST_F(hxvector_test_f, iteration) {
	{
		...
		hxvector<hxtest_object, 10u> objs;
		objs.push_back(hxtest_object(nums[0]));
		objs.push_back(hxtest_object(nums[1]));
		objs.push_back(hxtest_object(nums[2]));

		const hxvector<hxtest_object, 10u>& cobjs = objs;

		uint32_t counter = 0u;
		for(hxvector<hxtest_object, 10u>::iterator it = objs.begin(); it != objs.end(); ++it) {
		...
		counter = 0u;
		for(hxvector<hxtest_object, 10u>::const_iterator it = cobjs.begin();
				it != cobjs.end(); ++it) {
			EXPECT_EQ(it->id, objs[counter].id);
			EXPECT_EQ(it->id, nums[hxmin<uint32_t>(counter, 2u)]);
...
TEST_F(hxvector_test_f, get) {
	{
		hxvector<hxtest_object, 4u> objs; objs.reserve(4u);
		objs.emplace_back(10);
		objs.emplace_back(20);

		const hxvector<hxtest_object, 4u>& cobjs = objs;
...
// This is a regression test for hxvector::erase_if_heap.
...
TEST(hxvector_test, for_each_invokes_callables) {
	...
	// Count the objects with a non-temporary callable.
...
TEST_F(hxvector_test_f, assignment) {
	const hxsystem_allocator_scope temporary_stack_scope(hxsystem_allocator_temporary_stack);
	{
		...
		hxvector<hxtest_object> objs4(objs); // Construct from the same type.
		hxvector<hxtest_object, 1> objs5(objs); // Construct from a different type.
...
TEST_F(hxvector_test_f, assign_range_from_rvalue) {
	...
	elements.assign_range(hxvector_test_pointer_range(
...
TEST_F(hxvector_test_f, assign_range_from_const) {
	...
	hxvector_test_pointer_range<const hxtest_object> range(
...
TEST_F(hxvector_test_f, assign_range_from_mutable_range) {
	...
	hxvector_test_pointer_range<hxtest_object> range(
		source_elements, source_elements + source_count);
	elements.assign_range(range);
...
TEST(hxvector_test, cbegin_cend) {
	hxvector<int, 4u> values;
	values.push_back(1);
	values.push_back(3);
	values.push_back(5);

	const hxvector<int, 4u>& const_values = values;
	const int expected[] = { 1, 3, 5 };
	const size_t expected_count = hxsize(expected);
	size_t index = 0u;

	for (hxvector<int, 4u>::const_iterator it = const_values.cbegin(); it != const_values.cend(); ++it) {
...
// std::initializer_list is great for writing test code for hxvector. Not sure
...
TEST(hxvector_test, swaps) {
	...
	hxvector<int> y = hxmove(x); // Should swap.
	hxvector<int> z;
	hxswap(y, z);
	EXPECT_TRUE(x.empty());
	EXPECT_TRUE(y.empty());
