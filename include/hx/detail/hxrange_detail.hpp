...
/// \cond HIDDEN
// Tag dispatch on three_way_ so that a plain bool-returning less callable is
// never type-checked against `< 0`, and a three-way callable is never used as
// a bool. hxif_constexpr alone cannot discard the other branch before C++17.
// Used by hxflat_map and hxflat_set to share one binary search implementation
// between a strict weak order and a three-way compare_t.
...
// Tag dispatch on three_way_ for the equality recheck after a
// hxlower_bound_search_ call, for the same reason as hxcompare_before_ above.
// Only three_way_ can report equality at zero extra cost as part of the
...
// When three_way_ is true the found flag comes from the last "not before"
// step at zero extra cost, reusing the comparison the search already made
// against the position it converges on. When three_way_ is false a strict
// weak order cannot report equality from a single compare_ call, so one extra
// hxcompare_equal_ call is made once the search converges.
template<typename range_t_, typename value_t_, typename compare_t_, bool three_way_>
...
auto hxlower_bound_search_(range_t_&& range_, const value_t_& value_, const compare_t_& compare_)
	...
	hxif_constexpr(!three_way_) {
		found_ = begin_ != end_ && hxcompare_equal_<three_way_>::compare(compare_, value_, *begin_);
	}
	else {
...
// dispatches on three_way_ instead of requiring a strict weak order, for use
// by hxflat_map and hxflat_set.
template<typename range_t_, typename value_t_, typename compare_t_, bool three_way_>
hxattr_nodiscard hxinline hxconstexpr hxattr_flatten
auto hxupper_bound_(range_t_&& range_, const value_t_& value_, const compare_t_& compare_)
	...
	while(count_ > hxsize_t{0}) {
		const hxsize_t step_ = count_ >> 1;
		const iterator_t_ mid_ = begin_ + step_;
		if(hxcompare_after_<three_way_>::compare(compare_, value_, *mid_)) {
			begin_ = mid_ + hxsize_t{1};
			count_ -= step_ + hxsize_t{1};
		}
		else {
