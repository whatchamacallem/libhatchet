...
template<hxarray_element_concept_ T_, size_t capacity_=hxallocator_dynamic_capacity>
...
public:
	...
	hxattr_nodiscard size_t size(void) const { return this->capacity(); }

	/// Returns the number of bytes in the array. (Non-standard.)
	hxattr_nodiscard size_t size_bytes(void) const { return sizeof(T_) * this->capacity(); }
...
// The array overloads of hxkey_equal and hxkey_less are C++20 only. Without the
// "requires" keyword these end up being ambiguous. Use hxarray::equal and
// hxarray::less. Use C++20 if you want to use arrays as generic keys.
#if HX_CPLUSPLUS >= 202002L
