...
public:
	...
	/// Assigns nothing as key is immutable. Hash table linkage of either node
	/// is not affected.
...
/// copying data. Each bucket uses an embedded singly-linked list. Hash tables
/// can act as either an unordered map or an unordered set and support
/// operations that allow for unique or duplicate keys. While this interface is
/// designed to feel familiar, code using standard containers will need some
/// adjustments. In particular, table modification is non-standard. Iterators
/// are provided but are not used to modify the table.
///
...
public:
	...
	/// referenced. NOTA BENE: The iterators will automatically convert
	/// themselves to pointers when used in pointer context.
	class const_iterator
	{
	public:
		/// Constructs an iterator pointing to the end of the hash table.
		const_iterator(void) : m_hash_table_(hxnull), m_next_index_(0u), m_current_node_(hxnull) { }
	...
	/// Inserts `ptr` into the hash table. When `multi_t` is `false` and a node
	/// with an equal key already exists, returns an iterator to the existing node
	/// and the deleter is invoked on `ptr`. Otherwise inserts `ptr` and returns
	/// an iterator to it.
	/// - `ptr` : The `node_t` to insert. Must not be null.
	iterator insert(hxptr<node_t_, deleter_t_>&& ptr_);
...
// hxhash_table::const_iterator out-of-line implementations
...
template<hxhash_table_concept_ node_t_, hxhash_t table_size_bits_, bool multi_t_, typename deleter_t_>
inline typename hxhash_table<node_t_, table_size_bits_, multi_t_, deleter_t_>::const_iterator&
hxhash_table<node_t_, table_size_bits_, multi_t_, deleter_t_>::const_iterator::operator++(void) {
...
inline hxenable_if_t<multi_, typename hxhash_table<node_t_, table_size_bits_, multi_t_, deleter_t_>::iterator>
hxhash_table<node_t_, table_size_bits_, multi_t_, deleter_t_>::emplace(args_t_&&... args_) {
	node_t_* const ptr_ = hxnew<node_t_, allocator_, align_>(hxforward<args_t_>(args_)...);
	return this->insert(hxptr<node_t_, deleter_t_>(ptr_));
}
...
hxhash_table<node_t_, table_size_bits_, multi_t_, deleter_t_>::insert(hxptr<node_t_, deleter_t_>&& ptr_)
{
...
template<hxsystem_allocator_t allocator_, hxalignment_t align_, bool multi_, class... args_t_>
inline hxenable_if_t<!multi_, typename hxhash_table<node_t_, table_size_bits_, multi_t_, deleter_t_>::iterator>
hxhash_table<node_t_, table_size_bits_, multi_t_, deleter_t_>::try_emplace(
